...
for i=1:2
    ...
    beta1D(i,:) = [A1D(i), x01D(i), sig1D(i), 0];
    fp1D(i,:) = lsqcurvefit(@x_gaussian,beta1D(i,:),data1D{i},data1D{i},[0 0 0 -inf],[inf inf inf inf],options);
    fit1D{i} = x_gaussian(fp1D(i,:),data1D{i});
end
...
c = mean([fp1D(1,4)/numel(data1D{2}) fp1D(2,4)/numel(data1D{1})]);

fp = [A x0 y0 sigmax sigmay c 0];
...
elseif get(hfig_main.nc_fitx,'value')
    N = hfig_main.calculation.A/hfig_main.calculation.s_lambda*(sum(sum(data))-fp1D(1,4)*numel(data1D{1}));
elseif get(hfig_main.nc_fity,'value')
    N = hfig_main.calculation.A/hfig_main.calculation.s_lambda*(sum(sum(data))-fp1D(2,4)*numel(data1D{2}));
...
plot(hfig_dispCombined.xden_axes, x, data1D{1}/A1D(1), '.b', x, fit1D{1}/A1D(1), '-r','LineWidth',2)
